...
} from '../types/AvatarListing/AvatarListing'
// import {
...
import { Avatar, ReserveListing } from '../types/schema'
...
export function handleReserveListingCreated(event: ListingCreated): void {
    ...
    let intermediary = findOrCreateUser(event.params.intermediary.toHexString())
	  ...
	  let avatar = Avatar.load(token) 

	  //let avatarContract = AvatarContract.bind(event.params.tokenContract)
	  //let avatarExchangeAddress = avatarContract.avatarExchangeContract()
    ...
    createReserveListing(
        ...
        //avatarExchangeAddress.toHexString(),
        avatar,
